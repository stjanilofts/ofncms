...
class NewsController extends ItemableController
{
    ...
    public function greinar()
    {
        $data['title'] = 'Greinar';
        $data['items'] = \App\News::where('status', '>', 0)->where('type', 'article')->orderBy('published_at', 'DESC')->orderBy('order')->get();
        return view('frontend.overview')->with($data);
    }
    
    public function namskeid()
    {
        $data['title'] = 'Námskeið';
        $data['items'] = \App\News::where('status', '>', 0)->where('type', 'seminar')->orderBy('published_at', 'DESC')->orderBy('order')->get();
        return view('frontend.overview')->with($data);
    }

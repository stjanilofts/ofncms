...
@else
	<div id="{{ $field }}">
		...
		<ul class="uk-sortable {{ $field }} uk-grid uk-grid-small uk-grid-width-medium-1-4 uk-grid-width-large-1-6 uk-grid-width-small-1-3" v-class="disable-pointer-events: isUploading" data-uk-sortable>
			<li class="uk-grid-margin" v-repeat="image: {{ $field }}" style="cursor: move;" data-file-name="@{{ image.name }}" data-file-title="@{{ image.title }}">
				<div v-if="image.name" class="uk-panel uk-panel-box">
					...
					<div>
						<form class="uk-form uk-margin-top">
							<input class="uk-margin-small-bottom" v-on="blur: reorderImages(), blur: tester" v-model="image.title"><br>
			...
			var {{ $field }}View = new Vue({
				...
				methods: {
					...
					deleteImage: function(idx) {
						...
						UIkit.modal.confirm("Ertu viss um að þú viljir eyða?", function() {
							self.isUploading = true;
				   			...
				   			self.$http.post('/admin/formable/_deleteImage', {{ $field }}_data, function (data, status, request) {
								self.updateImageList();
							});
						});
					},
					...
					updateImageList: function() {
						this.$http.post('/admin/formable/_images', this.item, function (data, status, request) {
